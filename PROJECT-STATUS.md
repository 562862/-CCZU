...
- 远程：`https://github.com/562862/cczu-competition.git`
- 分支：`main`
...
| v1.3.1 | 待提交 | 学院源扩展到 29 个、修正学院映射、爬虫选择器回退增强 |

## v1.3.1 改动清单

### 学院源扩展与修正

- 爬虫源从 10 个扩展到 29 个（25 个二级学院 + 4 个校级部门）
- 修正 `cmee.cczu.edu.cn` 学院映射为“机械与轨道交通学院”
- 修正 `zfxy.cczu.edu.cn` 学院映射为“瞿秋白政府管理学院”

### 爬虫兼容性增强

- 新增条目选择器回退链：`li.news` / `li:has(a[href*=page])` / `tr.listnews_listbottomline` / `tr:has(table)`
- 新增标题、日期选择器回退链，降低不同学院模板差异导致的漏抓
- 分页 URL 构造兼容 `list.psp` 模式
...
> 最新 29 个爬虫源以 `src/main/resources/application.yml` 为准。
